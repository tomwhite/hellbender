...
public final class HaplotypeCallerSpark extends GATKSparkTool {
    ...
    public static JavaRDD<VariantContext> callVariantsWithHaplotypeCaller(
        ...
        final List<ShardBoundary> shardBoundaries = getShardBoundaries(header, intervals, shardingArgs.readShardSize);
    ...
    private static List<ShardBoundary> getShardBoundaries(final SAMFileHeader
        header, final List<SimpleInterval> intervals, final int readShardSize) {
        return intervals.stream()
            .flatMap(interval -> Shard.divideIntervalIntoShards(interval, readShardSize, 0, header.getSequenceDictionary()).stream())
